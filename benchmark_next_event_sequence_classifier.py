...
from train_next_event_cosine import NextEventCosineModel, collate_event_sequences
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Benchmark next-event sequence models on downstream patient classification tasks.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--checkpoint_paths", nargs="+", required=True,
                   help="Paths to train_next_event_cosine checkpoint directories containing model.pt.")
    ...
    p.add_argument("--output_dir", default="output/next-event-sequence-classifier-benchmark")
    return p.parse_args()
...
def encode_sequence_rows(
    ...
    for start in tqdm(range(0, len(event_lists), batch_size), desc=desc, dynamic_ncols=True):
        ...
        collated = collate_event_sequences(
            batch_events,
        ...
        seq_embs, _ = model.encode_sequence_embeddings(
            event_input_ids=collated["event_input_ids"],
            event_attention_mask=collated["event_attention_mask"],
            event_last_pos=collated["event_last_pos"],
...
def load_checkpoint_model(args, checkpoint_dir: str, device: torch.device):
    ...
    model = NextEventCosineModel(
        ...
        event_encoder_batch_size=int(train_args.get("event_encoder_batch_size", 0)),
...
def main():
    ...
    for checkpoint_label, checkpoint_path in zip(checkpoint_labels, args.checkpoint_paths):
        logger.info("=" * 100)
        logger.info("Benchmarking next-event checkpoint: %s -> %s", checkpoint_label, checkpoint_path)
        logger.info("=" * 100)
