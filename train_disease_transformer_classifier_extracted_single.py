...
class EventBatchCollator:
    ...
    def __call__(self, batch: list[tuple[np.ndarray, int]]) -> dict[str, torch.Tensor]:
        ...
        return collate_latest_leftpad(emb_list=emb_list, labels=labels, max_events=self.max_events)
...
class SingleTaskExtractedDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[np.ndarray, int]:
        ...
        if len(eids) == 0:
            return np.zeros((0, int(self.embeddings.shape[1])), dtype=np.float32), label
        return self.embeddings[eids], label


class BidirectionalTransformerBlock(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, intermediate_size: int, dropout: float, dtype: torch.dtype):
        super().__init__()
...
class EventTransformerClassifier(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.block = BidirectionalTransformerBlock(
    ...
    def forward(self, event_embs: torch.Tensor, event_mask: torch.Tensor, return_features: bool = False):
        ...
        if seq_len > self.max_positions:
            raise ValueError(f"Event sequence length {seq_len} exceeds max_positions={self.max_positions}")
        pos_ids = torch.arange(seq_len, device=x.device)
        pos = self.pos_emb(pos_ids).unsqueeze(0)
        x = x + pos * event_mask.to(x.dtype).unsqueeze(-1)
        key_padding_mask = ~event_mask.bool()
        hidden = self.block(x, key_padding_mask=key_padding_mask)
        mask = event_mask.to(hidden.dtype).unsqueeze(-1)
        pooled = (hidden * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1.0)
        logits = self.out_proj(self.out_norm(pooled)).squeeze(-1).float()
        if return_features:
            return logits, pooled.float()
        return logits
    ...
    @classmethod
    def load_checkpoint(cls, save_dir: Path, *, device: torch.device, dtype: torch.dtype) -> "EventTransformerClassifier":
...
def evaluate_single_task(
    ...
    *,
    ...
    all_logits = []
    all_labels = []
    for batch in tqdm(eval_loader, desc=f"Evaluating {task}/{eval_split}", dynamic_ncols=True):
        logits = model(batch["event_embs"].to(device), batch["event_mask"].to(device))
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Single-task event-only 1-layer bidirectional transformer classifier",
    ...
    p.add_argument("--output_dir", default="output/disease-transformer-classifier-extracted-single")
...
def main():
    ...
    logger.info("Single-task transformer mode: task=%s", args.task)
    ...
    logger.info(
        ...
        train_stats["num_samples"], train_stats["num_pos"], train_stats["num_neg"],
        train_stats["num_pos"] / max(train_stats["num_neg"], 1),
    )
    logger.info(
        "Eval stats (%s): n=%d pos=%d neg=%d pos/neg=%.4f",
        args.eval_split, eval_stats["num_samples"], eval_stats["num_pos"], eval_stats["num_neg"],
    ...
    if args.checkpoint:
        model = EventTransformerClassifier.load_checkpoint(Path(args.checkpoint), device=device, dtype=dtype)
    else:
        model = EventTransformerClassifier(
            ...
            max_positions=args.inferred_max_events,
            dropout=args.dropout,
            dtype=dtype,
        ).to(device)
    logger.info(
        "1-layer transformer classifier: hidden=%d heads=%d intermediate=%s max_positions=%d dropout=%.3f dtype=%s",
        args.hidden_size,
        args.num_heads,
    ...
    for epoch_idx in range(args.epochs):
        ...
        optimizer.zero_grad()
        epoch_loss = 0.0
        pbar = tqdm(train_loader, desc=f"Epoch {epoch_idx + 1}/{args.epochs}", dynamic_ncols=True, total=steps_per_epoch)
